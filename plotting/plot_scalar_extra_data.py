...
def generate_recons_quality_stats(true_imgs,noisy_imgs,recons):
    ...
    ssims = []
    for i in range(n_samples):
        l2_costs.append(0.5*np.linalg.norm(true_imgs[:,:,i].ravel() - recons[:,:,i].ravel())**2)
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python %s settings_file results_folder outfolder" % sys.argv[0])
        print("where")
        print("    settings_file = json file containing run details")
        print("    results_folder = folder where results are to")
    ...
    outfolder = os.path.join(sys.argv[3], settings_file_basename)
    ...
    plot_recons(opt_param,true_imgs,noisy_imgs,recons,px,py)
