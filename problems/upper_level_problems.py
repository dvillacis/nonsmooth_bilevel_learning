...
class UpperScalarDataLearning_2D(UpperLevelProblem):
    def __init__(self, num_training_data, seed = 0, noise_level=0.1, npixels=20, verbose = False):
        ...
        for i in range(num_training_data):
            true_img = get_2D_phantom(i,height=npixels,width=npixels)
...
class UpperPatchDataLearning_2D(UpperLevelProblem):
...
def main():
    up_prob = UpperPatchRegLearning_2D(3,1,1)
    for i in np.arange(1e-5,1.0,1e-2):
        up_prob(i,smooth=False) 
